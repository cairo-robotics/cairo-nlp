...
class NLPGoogleServer():

    def __init__(self):
        rospy.init_node("nlp_google_server")
        # Create a repeater for user input
        self.pub = rospy.Publisher("/nlp/google_server",
    ...
    def run(self):
        ...
        """
        rospy.Service("/nlp/google/tts", TTS, self.textToSpeech)
        rospy.Service("/nlp/google/stt", STT, self.speechToText)
        rospy.loginfo("NLP Google Server: Starting...")
    ...
    def textToSpeech(self, tts_srv):
        ...
        success = True
        rospy.loginfo("NLP Google Server TTS: Computing speech from text: %s",
                      tts_srv.input)
        ...
        try:
            ...
            rospy.logerr("NLP Google Server TTS: %s", str(ex))
            success = False

        # Publish robot speech
        msg = String()
        msg.data = "TTS: " + tts_srv.input.lower()
    ...
    def speechToText(self, stt_srv):
        ...
        response_text = ""
        rospy.loginfo("NLP Google Server STT: Computing text from speech: %s",
                      stt_srv.input)
        try:
            ...
            rospy.logerr("NLP Google Server STT: %s", str(ex))

        # Return the text
        msg = String()
        msg.data = "STT: " + response_text.lower()
        self.pub.publish(msg)
